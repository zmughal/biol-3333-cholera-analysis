...
plot.all.delta.cases <- ggplot(tracking.agg, aes(Date,CleanDeltaCases)) + geom_line()
#print(plot.all.delta.cases)
ggsave( file.path('graphs', 'plot.all.delta.cases.png'))

plot.all.delta.deaths <- ggplot(tracking.agg, aes(Date,CleanDeltaDeaths)) + geom_line()
